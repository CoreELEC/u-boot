...
static void getvar_product(char *var_parameter, char *response)
{
	...
	if (busy_flag == 1) {
		if (board)
			fastboot_response("INFO product: ", response, "%s", board);
		else
...
static void getvar_platform(char *var_parameter, char *response)
{
	...
	if (busy_flag == 1) {
		if (p)
			fastboot_response("INFOplatform: ", response, "%s", p);
		else
			fastboot_busy("platform not set", response);
	} else {
		if (p)
...
static void partition_type_reply(char *part_name, char *response, char *part_type)
{
	...
	if (busy_flag == 1) {
		if (rc == -1)
			...
			fastboot_response("INFO partition-type:", response, "%s: %s", part_name, part_type);
	} else {
		if (rc == -1)
...
static void getvar_partition_type(char *part_name, char *response)
{
	if ((strcmp_l1(part_name, "system") == 0) || (strcmp_l1(part_name, "vendor") == 0) ||
		...
		if (is_f2fs_by_name("data") == -1 && is_f2fs_by_name("userdata") == -1) {
			...
			printf("data is ext4\n");
			if (busy_flag == 1)
				fastboot_response("INFO partition-type:", response,
							"%s: ext4", part_name);
			else
		...
		if (has_boot_slot == 0) {
			if (busy_flag == 1)
				fastboot_response("INFO partition-type:", response, "%s: ext4", part_name);
			else
				fastboot_okay("ext4", response);
		} else {
			if (busy_flag == 1)
...
static void getvar_partition_size(char *part_name, char *response)
{
	...
	if (r >= 0) {
		if (busy_flag == 1) {
			char all_name[64];
			strcpy(all_name, "INFO partition-size:\0");
