 ...
 */
int v2_key_read(const char *keyName, u8 *keyVal, const unsigned int keyValLen, char *errInfo, unsigned int *fmtLen);
 ...
 *@errInfo: start it with success if burned ok, or format error info into it tell pc burned failed
 */
unsigned int v2_key_burn(const char *keyName, const u8 *keyVal, const unsigned int keyValLen, char *errInfo);
...
unsigned int add_sum(const void *pBuf, const unsigned int size);//Add-sum used for 64K transfer
...
int platform_busy_increase_un_reported_size(const unsigned int nBytes);
...
int bootloader_write(u8 *dataBuf, unsigned int off, unsigned int binSz);
