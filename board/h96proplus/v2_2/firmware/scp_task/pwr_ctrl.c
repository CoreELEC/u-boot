...
static void power_off_at_clk81(unsigned int shutdown)
{
	if(shutdown == SYS_POWEROFF) {
		hdmi_5v_ctrl(OFF);
	}
...
static void power_off_at_24M(unsigned int shutdown)
{
	...
	if(shutdown == SYS_SUSPEND) {
		aml_update_bits(PREG_PAD_GPIO0_EN_N, 1 << 28, 0);
		aml_update_bits(PREG_PAD_GPIO0_O, 1 << 28, 1 << 28);
	}
}
...
static void power_on_at_24M(void)
{
	...
	/* RED LED GPIODV_28*/
	aml_update_bits(PREG_PAD_GPIO0_EN_N, 1 << 28, 0);
	aml_update_bits(PREG_PAD_GPIO0_O, 1 << 28, 0);
}
