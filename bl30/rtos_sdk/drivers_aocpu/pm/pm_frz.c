...
static SemaphoreHandle_t xPM_wakeup_sem;
...
__weak void platform_power_interface_register(void)
{
	logi("Platform power not register..\n");
}
...
int find_static_power_dev(void)
{
	...
	if (xTaskCreate(pm_main, "pm_task", configMINIMAL_STACK_SIZE,
				NULL, 3, &pm_main_handle) < 0)
