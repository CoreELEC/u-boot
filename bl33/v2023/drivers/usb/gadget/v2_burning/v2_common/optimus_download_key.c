...
static void hdcp2DataDecryption(const unsigned int len, const char *input, char *out)
{
	int i = 0;
...
unsigned int v2_key_burn(const char *keyName, const u8 *keyVal, const unsigned int keyValLen, char *errInfo)
{
	...
	if (hdcprx22KeyIndex < _HDCP22RxTypes) {
		...
		const unsigned int gensum = add_sum(keyVal + 4, keyValLen - 4);
		int i = 0;
 ...
 *@errInfo: start it with success if burned ok, or format error info into it tell pc burned failed
 */
int v2_key_read(const char *keyName, u8 *keyVal, const unsigned int keyValLen, char *errInfo, unsigned *fmtLen)
{
	ssize_t keysize = 0;
	int rc = 0;
