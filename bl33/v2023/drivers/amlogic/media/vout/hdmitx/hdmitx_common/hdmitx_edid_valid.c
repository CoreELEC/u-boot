...
static bool hdmitx21_edid_validate_mode(struct hdmitx_dev *hdev, enum hdmi_vic vic)
{
	int i;
	bool ret = false;
...
enum hdmi_vic hdmitx21_get_prefer_vic(struct hdmitx_dev *hdev, enum hdmi_vic vic)
{
	...
	for (i = 0; i < ARRAY_SIZE(vic_pairs); i++) {
		if (vic_pairs[i].mode_alternate_vic == vic || vic_pairs[i].mode_prefer_vic == vic) {
			if (hdmitx21_edid_validate_mode(hdev, vic_pairs[i].mode_prefer_vic))
				return vic_pairs[i].mode_prefer_vic;
			if (hdmitx21_edid_validate_mode(hdev, vic_pairs[i].mode_alternate_vic))
