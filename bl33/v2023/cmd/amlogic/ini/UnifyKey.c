...
static char _g_dev_path[256] = {0};
static int get_dev_path(char path_buf[])
{
	int tmp_len = 0;
	FILE *dev_fp = NULL;
...
static const char *get_key_data_write_dev_path(void)
{
	memset((void *)_g_dev_path, 0, 256);
	get_dev_path(_g_dev_path);
	return _g_dev_path;
}

static const char *get_key_data_read_dev_path(void)
{
	memset((void *)_g_dev_path, 0, 256);
	get_dev_path(_g_dev_path);
	return _g_dev_path;
...
static int check_key_name_in_list(const char *key_name)
{
	FILE *dev_fp = NULL;
	char *tmp_ptr = NULL;
	char lineStr[1024];
...
int read_ukey_data_no_header(const char *key_name, unsigned char data_buf[], int rd_size)
{
	...
	if (check_key_name_in_list(key_name) < 0) {
	...
	if (mode == 0) {
		dev_fp = fopen(get_key_data_read_dev_path(), "r");
		if (!dev_fp) {
			ALOGE("%s, open %s ERROR(%s)!!\n", __func__,
			      get_key_data_read_dev_path(), strerror(errno));
			return -1;
		}
		...
		dev_fp = fopen(get_key_data_read_dev_path(), "rb");
		if (!dev_fp) {
			ALOGE("%s, open %s ERROR(%s)!!\n", __func__,
			      get_key_data_read_dev_path(), strerror(errno));
			return -1;
...
int write_ukey_data(const char *key_name, unsigned char data_buf[], int wr_size)
{
	...
	dev_fd = open(get_key_data_write_dev_path(), O_WRONLY | O_SYNC | O_CREAT | O_TRUNC, S_IRWXU | S_IRWXG | S_IRWXO);

	if (dev_fd < 0) {
		ALOGE("%s, open %s ERROR(%s)!!\n", __func__,
		      get_key_data_write_dev_path(), strerror(errno));
		return -1;
	}
...
static int check_UnifyKey(const char *key_name)
{
...
int read_ukey_data_no_header(const char *key_name, unsigned char data_buf[], int rd_size)
{
	...
	key_len = check_UnifyKey(key_name);
	if (key_len < 0) {
		return -1;
	} else if (key_len == 0) {
...
int key_unify_write(const char *keyname, const void *keydata, const unsigned datalen)
{
	...
	tmp_ret = write_ukey_data(keyname, (unsigned char *)keydata, datalen);
	if (tmp_ret != datalen)
		return -1;
...
{
	if (read_ukey_data(keyname, (unsigned char *)keydata, CC_ONE_SECTION_SIZE) <= 0)
		return -1;
...
int key_unify_query_size(const char *keyname, ssize_t *keysize)
{
	...
	rd_size = read_ukey_data(keyname, gTempBuf, CC_ONE_SECTION_SIZE);
	if (rd_size > 0) {
		*keysize = rd_size;
		return 0;
	}
...
{
	if (check_key_name_in_list(keyname) < 0) {
		ALOGE("%s, key \"%s\" isn't exist in unifykeys list\n", __func__, keyname);
		*exist = 0;
		return -1;
	}
