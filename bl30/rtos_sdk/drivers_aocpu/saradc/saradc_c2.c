...
static inline void prvAdcEnableChannel(enum AdcChannelType ch, uint8_t idx)
{
	...
	REG32(P_SARADC(SARADC_C2_CH0_CTRL2 + ch * 12)) = SARADC_C2_DISCARD_DATA_CNT;
	REG32(P_SARADC(SARADC_C2_CH0_CTRL3 + ch * 12)) = SARADC_C2_SAVE_DATA_CNT;
}
...
void vAdcSelfTest(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(ch7Vol); i++) {
		REG32_UPDATE_BITS(P_SARADC(SARADC_C2_CH0_CTRL1 + SARADC_CH7 * 12),
