...
unsigned int ss_freq_max = sizeof(lcd_ss_freq_table_t5) / sizeof(char *);
unsigned int ss_mode_max = sizeof(lcd_ss_mode_table_t5) / sizeof(char *);
...
void lcd_get_ss(struct aml_lcd_drv_s *pdrv)
{
	...
	printf("ss_freq: %s\n", lcd_ss_freq_table_t5[temp]);
	temp = (cconf->ss_mode >= ss_mode_max) ? 0 : cconf->ss_mode;
	printf("ss_mode: %s\n", lcd_ss_mode_table_t5[temp]);
}
...
void lcd_update_clk(struct aml_lcd_drv_s *pdrv)
{
	...
	if (cconf->data->clk_frac_update)
		cconf->data->clk_frac_update(pdrv);
...
void lcd_clk_config_chip_init(struct aml_lcd_drv_s *pdrv, struct lcd_clk_config_s *cconf)
{
	...
	ss_level_max = (cconf->data->ss_support == 1) ?
				sizeof(lcd_ss_level_table_t5) / sizeof(char *) : ss_level_max;
