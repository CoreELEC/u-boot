...
static uint64_t* base_xlation_table __attribute__ ((section(".xlatable")));
					...
					unsigned long long max_addr)
{
	...
	configASSERT((max_addr & ADDR_MASK_48_TO_63) == 0);
...
void init_xlat_tables(void)
{
	...
	uintptr_t max_va;
	print_mmap();
	base_xlation_table = early_reserve_pages(XLAT_TABLE_SIZE);
	configASSERT(base_xlation_table!=NULL);
	init_xlation_table(0, base_xlation_table, XLAT_TABLE_LEVEL_BASE,
			   &max_va, &max_pa);
	tcr_ps_bits = calc_physical_addr_size_bits(max_pa);
	configASSERT(max_va < ADDR_SPACE_SIZE);
}
#if ENABLE_KASAN
#define XLAT_TABLE_BASE_SHIFT (L0_XLAT_ADDRESS_SHIFT - XLAT_TABLE_LEVEL_BASE * XLAT_TABLE_ENTRIES_SHIFT)
#define XLAT_TABLE_BASE_MASK ((1UL<<XLAT_TABLE_BASE_SHIFT)-1)
#define XLAT_TABLE_BASE(a) (&base_xlation_table[(((unsigned long)(a))>>XLAT_TABLE_BASE_SHIFT)&XLAT_TABLE_ENTRIES_MASK])
void init_kasan_xlat_tables(mmap_region_t *mm,
			uintptr_t base_va,
			uintptr_t max_va)
{
	init_kasan_xlation_table(mm, base_va&~XLAT_TABLE_BASE_MASK, max_va,
			XLAT_TABLE_BASE(base_va),
...
#define DEFINE_ENABLE_MMU_EL(_el, _tcr_extra, _tlbi_fct)		\
	...
	{								\
		...
		uint32_t sctlr;						\
									\
		configASSERT(IS_IN_EL(_el));					\
		configASSERT((read_sctlr_el##_el() & SCTLR_M_BIT) == 0);	\
									\
