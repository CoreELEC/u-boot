...
#if defined(SARADC_REG11_BANDGAP_EN_BIT)
#define SARADC_REG11_BANDGAP_EN BIT(SARADC_REG11_BANDGAP_EN_BIT)
#else
...
void vAdcInit(void)
{
	...
	REG32_UPDATE_BITS(P_SARADC(SARADC_REG11), SARADC_REG11_VREF_SEL, SARADC_REG11_VREF_SEL);
...
void vAdcHwEnable(void)
{
	...
	REG32_UPDATE_BITS(P_SARADC(SARADC_REG11),
			  SARADC_REG11_CMV_SEL | SARADC_REG11_VREF_EN | SARADC_REG11_EOC |
...
void vAdcSelfTest(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(ch7Vol); i++) {
		REG32_UPDATE_BITS(P_SARADC(SARADC_REG3), SARADC_REG3_CTRL_CHAN7_MUX_SEL_MASK,
