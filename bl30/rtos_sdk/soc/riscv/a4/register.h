...
#define REGISTER_H
	...
	Wr(reg, ((Rd(reg) & ~(((1L<<(len))-1)<<(start))) | \
				((uint32_t)(val & ((1L<<len)-1)) << (start))))
#else
    #include "dpi.h"
#endif
...
#define MIPI_DSI_PHY_CTRL                          ((0x0000  << 2) + 0xfe01c000)
  ...
  //bit 1. write 1 to sync the txclkesc input. the internal logic have to use txclkesc \
								to decide Txvalid and Txready.
...
#define MIPI_DSI_CHAN_CTRL                         ((0x0001  << 2) + 0xfe01c000)
  ...
  //bit 21.  chan0 contention detection enable control selection. 1 = from register. \
								0 from chan0 state machine.
...
#define MIPI_DSI_LPOK_TIM                          ((0x0009  << 2) + 0xfe01c000)
  ...
  //every MIPI_DSI_ULPS_CHECK period, the receiver would be enabled once, and waiting \
						this timer period to get the stable input.
...
#define PDM_CTRL                                   ((0x0000  << 2) + 0xfe331000)
	...
	//bit 28:   bypass mode. 1: bypass all filter. directly output the PDM input \
				to DDR. 0 : normal mode.
	//bit 27~9: not used.
...
#define PDM_F1_CTRL                                ((0x0003  << 2) + 0xfe331000)
	...
	//bit 16:15. f1 round mode. 2'b00 : sign bit at bit 49. 28bits output[49:22] \
			round at bit 21. 32bits output[49:18]. 24bits output[49:26].
	//2'b01 : sign bit at bit 50. 28bits output[50:23] round at bit 22. \
			32bits output[49:18]. 24bits output[49:26].
	//2'b10 : sign bit at bit 51. 28bits output[51:24] round at bit 23. \
			32bits output[49:18]. 24bits output[49:26].
...
#define PDM_CLKG_CTRL                              ((0x000b  << 2) + 0xfe331000)
  // auto clock gating control.  1: disable the clock gating function. \
			the clock will awlays enabled. 0 : use RTL auto clock gating.
  //31:7 not used.
...
//Bit     7:0,      comma_cnt_th            unsigned, default = 0, should bigger than 3 and \
										small than 10
...
//Bit        30,     manual_reset_enable           unsigned, default = 0, \
							manual reset select, 1 : manual
...
//Bit        30,     master_hb_ignore              unsigned, default =0, \
			0 : wait hb issued before pkt cmd, 1 : pkt cmd issued immediately
...
//Bit 31:0,     reg_spdif_mute_val     ,default = 0,when biahpase encode mute, \
					the channel value, with reg_mute_l/reg_mute_r
...
//Bit 26,     reg_userdata_sel    ,default = 0,user Bit select : \
						0 from reg_userdata_set 1 from data[29]
...
//Bit 16,     reg_mute_hold_init_en  ,default = 0,when c_mute_hold_last_err_corrt \
					valid, clear work enable, initial biphase encode
//Bit 15:12,  reserved
...
//Bit 7,      reg_force_start     ,default = 0,set 1 no need ack from frddr to \
...
//Bit 15,     reg_init_send_en    ,default = 0,send 01 sequence some times after \
							initial done from frddr set
...
//Bit 15:8,  reg_ubit_fifo_start_thd      ,default = 0,start transmit iu after fifo \
							level greater than this value
//Bit 7:0,   reg_ubit_fifo_less_thd       ,default = 0,generate irq,when fifo level \
...
//Bit 27,    reg_hold_soft_clr_en   ,default = 0,0 auto clear hold at next valid 1 \
						clear hold with reg_mute_hold_clr
...
//Bit   6,       reg_earctx_hd_hpd_value  unsigned, default = 0, earctx_hd_hdp mux = 3, \
										register value
...
//Bit   19:0,   reg_base_timer                unsigned, default = 0, base timer to detect \
									sample mode change
#define EARCRX_SPDIFIN_SAMPLE_CTRL1                ((0x0004  << 2) + 0xfe333c00)
//Bit   31,     reg_force_sample_mode         unsigned, default = 0, 0 auto detect sample \
					mode 1 force a fixed sample mode with reg_sample_mode
//Bit   30,     reserved
...
//Bit   23:19,  reg_mute_block_num             unsigned, default = 0, mute min block number \
										to declare
...
//Bit   16,     reg_earc_cps_chst_clr_en       unsigned, default = 0, auto clear compress \
							mode when channel status not compress
//Bit   15,     reg_earc_cps_nonpcm2pcm_clr_en unsigned, default = 0, auto clear compress \
									mode when nonpcm2pcm
//Bit   14,     reg_earc_auto                  unsigned, default = 0, auto change earc/arc
//Bit   13,     reg_earcin_papb_lr             unsigned, default = 0, user l or r channel \
									status to check papb
//Bit   12,     reg_earcin_check_papb          unsigned, default = 0, \
		0 : data valid after 1 block; 1 : in 1st block if exit papb, data valid after papb
...
//Bit   3,      reg_earcin_spdif_force        unsigned, default = 0, when in arc mode, \
								spdif on force enable
...
//Bit   23:16,  reg_fifo_thd                  unsigned, default = 0, generate irq when fifo \
								level pass some threshold
//Bit   15,     reg_max_dist_en               unsigned, default = 0, max distance between \
					IUs to set lost
...
//Bit   31,     reg_work_enable               unsigned, default = 0, analog reset check \
								work enable
...
//Bit   5,      reg_spdif_rx_sqen_force       unsigned, default = 0, force spdif_rx_sqen \
						to reg_spdif_rx_sqe
//Bit   4,      reg_spdif_rx_sqen_force_value unsigned, default = 0, value
//Bit   3,      reg_dmacrx_en_force           unsigned, default = 0, force dmacrx_en \
						to reg_dmacrx_en_force_value
//Bit   2,      reg_dmacrx_en_force_value     unsigned, default = 0, value
//Bit   1,      reg_dmacrx_sqen_force         unsigned, default = 0, force dmacrx_sqen \
						to reg_dmacrx_sqen_force_value
...
// Bit     1 ge2d_rd_src1 cmd_if.stat_read_window_err, \
		1 = reading/clipping window setting exceed limit
...
//Bit 31, SRC1 x start extra bit1, if true, one more chroma data is read for x even start \
	  chroma data when y/c ratio = 2 or x even/odd start chroma extra data when y/c ratio = 1
//Bit 30, SRC1 x start extra bit0, if true, one more chroma data is read for x odd start \
...
//Bit 15, SRC1 x end extra bit1, if true, one more chroma data is read for x odd end \
	  chroma data when y/c ratio = 2 or x even/odd end chroma extra data when y/c ratio = 1
...
//Bit 15, SRC1 y end extra bit1, if true, one more chroma data is read for y odd end \
	  chroma data when y/c ratio = 2 or y even/odd end chroma extra data when y/c ratio = 1
...
//Bit 23:0, horizontal scaler advance phase in this round, if horizontal scaler is \
	    working on dividing mode
...
//Bit 9, if true, treat horizontal repeat line number(GE2D_HSC_INI_CTRL bit 30:29) \
	 as repeating line,
...
//Bit 14	1 : read coef through cbus enable, just for debug purpose in case when \
