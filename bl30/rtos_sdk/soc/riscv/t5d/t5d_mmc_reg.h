...
// this setting is used for the synopsys DDR PHY PLL fast lock mode. bit 2. enable dmc_clk. bit 1.
// not used. bit 0. enable LPDDR4-PHY clock
...
// or it can auto changed by FREQ change hardware. bit 5:3   OD1. bit 2:0.  OD.
...
// bit 23:16 8 AMBUS input interface n_clk domain reset_n signal. 0 : reset.  1: normal working
// mode. bit 15:0. 16 AXI BUS input intefaces n_clk domain reset_n signal. 0: reset. 1: normal
// working mode. each bit for one interface.
...
// both n_clk and m_clk domain bit 31~24 not used. bit 23.16  8 am bus interfaces master clock
// domain reset_n signal. 0 : reset : 1 normal working mode. bit 15:0.  16 AXI bus interfaces
// master clock domain reset_n signal. 0 : reset : 1 normal working mode.
...
// bit 40:29.   canvas Width, unit: 8bytes. must in 32bytes boundary. that means last 2 bits must
// be 0. bit 28:0.    cavnas start address.   unit. 8 bytes. must be in 32bytes boundary. that
// means last 2bits must be 0.

#define DC_CAV_LUT_ADDR ((0x0014 << 2) + 0xff638000)
// bit 9:8.   write 9:8 2'b10. the canvas data will saved in canvas memory with address 7:0.
...
// there are total 256 canvas table, So we added a 256bits register to record the those canvas
// mode. when we configure the canvas table and if the canvas mode is 32x32 blkmode, we'll mark
// this related bit. we will use this bit to check the VPU canvas access. if it's in 32x32
// blkmode, we need break the input in 32bytes boundary. You can also use below register to
// read/write those bit.
...
// block the ambus related bits write request. bit 15:0  : when side band signal used as block
// other request, and side bank signal is high,  block the axi bus related bits write request.
...
// to ID = 0. Mali traffic will assigned to ID =1. 1 : Mali 2 : PCIE 3 : HDMI. 4 : HEVC_F. 5 :
// TEST. 6 : USB 7 : Device. 8 : HEVC_B 9 : WAVE.
// 10 : CPU, GPU and NNA. CPU will assign to ID = 13.  GPU and NNA still in ID = 10. we use the
// GPU/NNA ID to control the secure control. 11 : GDC. 12 : ISP.
...
// HEVC_F  9 bits. dc_id_in[7:4] == 0 : for IMEM. dc_id_in[7:4] == 1 : for PMEM. dc_id_in[7:5]  !=
// 0 for other 7 types. TEST  1 type. USB   1 type. DEVICE  AO_CPU : AR/AWID[7:3] == 4'ha.  4bits
// control.   use AR/WID[1:0].
...
// trap function: all the enable the port ID read access or enable PORT ID and subID read access
// must be in the predefine range. otherwire the read access would be blocked.
...
//         0: only if the apb access pprot[2:0]  match the bit [2:0], can access this domain
//         registers.
...
// bit 0. DMC_DDR_LOCK.    1: LOCK DMC_DDR_CTRL, DMC_DDR_CTRL1, DMC_AXI2DDRx, DDR0/1_ADDRMAP_x
