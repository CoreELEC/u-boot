...
struct mbPackInfo syncDspMbInfo;
...
static void vAoRevMbHandler(void *vArg)
{
	...
	PRINT_DBG("[%s]:Rev Handler 0x%lx, 0x%lx, 0x%lx\n", MBTAG, ulMbCmd, ulSize, ulSync);

	if (ulMbCmd == 0) {
		PRINT_DBG("[%s]:Err: mbox cmd is 0\n", MBTAG);
...
static void vReeSyncTask(void *pvParameters)
{
	...
	while (1) {
		...
		vEnterCritical();
		PRINT_DBG("[%s]:ReeSync clear mbox:%d\n", MBTAG, mbox);
		ulReeSyncTaskWake = 0;
		vClrMboxStats(MAILBOX_CLR(mbox));
		vClrMbInterrupt(IRQ_REV_BIT(mbox));
		vEnableMbInterrupt(IRQ_REV_BIT(mbox));
...
static void vTeeSyncTask(void *pvParameters)
{
	...
	while (1) {
		...
		if (index != 0) {
			if (index == MAX_ENTRY_NUM) {
				memset(&syncTeeMbInfo.mbdata.data, 0,
				       sizeof(syncTeeMbInfo.mbdata.data));
				syncTeeMbInfo.mbdata.status = ACK_FAIL;
				vBuildPayload(addr, &syncTeeMbInfo.mbdata,
					      sizeof(syncTeeMbInfo.mbdata));
				PRINT_DBG("[%s]: undefine cmd or no callback\n", MBTAG);
			} else {
				PRINT_DBG("[%s]:TeeSync re len:%d\n", MBTAG,
					  sizeof(syncTeeMbInfo.mbdata));
		...
		vEnterCritical();
		PRINT_DBG("[%s]:TeeSync clear mbox:%d\n", MBTAG, mbox);
		ulTeeSyncTaskWake = 0;
		vClrMboxStats(MAILBOX_CLR(mbox));
		vClrMbInterrupt(IRQ_REV_BIT(mbox));
		vEnableMbInterrupt(IRQ_REV_BIT(mbox));
...
static void vDspSyncTask(void *pvParameters)
{
	...
	while (1) {
		...
		if (index != 0) {
			if (index == MAX_ENTRY_NUM) {
				memset(&syncDspMbInfo.mbdata.data, 0,
				       sizeof(syncDspMbInfo.mbdata.data));
				syncDspMbInfo.mbdata.status = ACK_FAIL;
				vBuildPayload(addr, &syncDspMbInfo.mbdata,
					      sizeof(syncDspMbInfo.mbdata));
				PRINT_DBG("[%s]: undefine cmd or no callback\n", MBTAG);
			} else {
				PRINT_DBG("[%s]:DspSync re len:%d\n", MBTAG,
					  sizeof(syncDspMbInfo.mbdata));
		...
		vEnterCritical();
		PRINT_DBG("[%s]:DspSync clear mbox:%d\n", MBTAG, mbox);
		ulDspSyncTaskWake = 0;
		vClrMboxStats(MAILBOX_CLR(mbox));
		vClrMbInterrupt(IRQ_REV_BIT(mbox));
		vEnableMbInterrupt(IRQ_REV_BIT(mbox));
...
int vMbInit(void)
{
	...
	vSetMbIrqHandler(IRQ_REV_NUM(MAILBOX_ARMREE2AO), vAoRevMbHandler, (void *)MAILBOX_ARMREE2AO,
			 10);

	vSetMbIrqHandler(IRQ_REV_NUM(MAILBOX_ARMTEE2AO), vAoRevMbHandler, (void *)MAILBOX_ARMTEE2AO,
			 10);

	vSetMbIrqHandler(IRQ_REV_NUM(MAILBOX_DSPA2AO), vAoRevMbHandler, (void *)MAILBOX_DSPA2AO,
											10);
	vSetMbIrqHandler(IRQ_SENDACK_NUM(MAILBOX_AO2DSPA), vAoAckMbHandler,
								(void *)MAILBOX_AO2DSPA, 10);
	...
	xTaskCreate(vReeSyncTask, "AOReeSyncTask", configMINIMAL_STACK_SIZE, 0, TASK_PRIORITY,
		    (TaskHandle_t *)&ReembHandler);
	xTaskCreate(vTeeSyncTask, "AOTeeSyncTask", configMINIMAL_STACK_SIZE, 0, TASK_PRIORITY,
		    (TaskHandle_t *)&TeembHandler);
	xTaskCreate(vDspSyncTask, "AODspSyncTask", configMINIMAL_STACK_SIZE, 0, TASK_PRIORITY,
		    (TaskHandle_t *)&DspmbHandler);

	ret = vRpcUserCmdInit();
	PRINT("[%s]: mbox init end\n", MBTAG);
	return ret;
}
						 ...
						 void *(*handler)(void *), uint8_t needFdBak)
{
	...
	if (ret == MAX_ENTRY_NUM) {
		PRINT_ERR("[%s]: mbox cmd 0x%x reg fail\n", MBTAG, cmd);
				 ...
				 void *data, size_t size)
{
	...
	mbSize = size + sizeof(struct mbHeadInfo);
	...
	if (xPreVal == 0) {
		PRINT_ERR("[%s]:Err: mbox XEFR msg TMO!\n", MBTAG);
		st.cmd = 0;
		st.size = 0;
		st.sync = 0;
		vSetMboxStats(MAILBOX_SET(mboxId), st);
