...
static int split_file_path(const char *file_path, char part_name[], char file_name[], const char *ext_name)
{
	int i = 0;
...
int ini_is_file_exist(const char *file_path)
{
	...
	if (split_file_path(file_path, part_name, file_name, NULL) < 0)
		return 0;

	if (set_block_device(part_name) < 0)
		return 0;
