...
typedef int (*pFunc_getKeyValByLic)(const char *licensePath, u8 *keyVal, unsigned int *keyValLen, const char *keyName);
...
typedef struct _keyFmtMapping{
	...
	const unsigned int         keyFmt;
	pFunc_getKeyValByLic    get_key_val;
	pFunc_updateLic         update_license;
} KeyFmtMap_t;

static int get_key_val_for_fmt_mac(const char *licenseName, u8 *keyVal, unsigned int *keyValLen, const char *keyName);
static int get_key_val_for_fmt_hdcp(const char *licenseName, u8 *keyVal, unsigned int *keyValLen, const char *keyName);
static int get_key_val_for_fmt_hdcp2rx(const char *licenseName, u8 *keyVal, unsigned int *keyValLen, const char *keyName);
static int get_key_val_for_fmt_onlyone(const char *licenseName, u8 *keyVal, unsigned int *keyValLen, const char *keyName);
...
static int optimus_sdc_burn_check_mac_ini_set_is_valid(const char *setName)
{
	...
	const int   nSet      = ARRAY_SIZE(macSets);
	int         index     = 0;
	int         rc        = 0;
...
typedef struct {
	...
	const char  	*end;
	unsigned int	used;
	unsigned int	total;//total = end + 1 - start
} MacCfg_t;

/*static KeyInf_mac_t _bootPart_keyInf_Mac;//Mac key info stored in boot partition*/

static int optimus_update_mac_str_current_val(char *startVal, unsigned int used)
{
	int i = 0;

	if (strlen(startVal) != 17) {
	...
	for (i = 5; i >= 0 && used; --i, (used >>= 8)) {
		...
		unsigned int grp2Val = simple_strtoul(grp2, NULL, 16);
...
static int _optimus_get_mac_diff(MacCfg_t *macCfg)
{
	...
	DWN_MSG("mac range[%04x%08x, %04x%08x]\n",
		(unsigned int)(startVal >> 32), (unsigned int)startVal,
		(unsigned int)(endVal >> 32),   (unsigned int)endVal);
	macCfg->total = (unsigned int)(endVal + 1 - startVal);
	return 0;
}

//update the mac value extracted from mac.ini
//To simplest the case, currently one group is supported
static int optimus_sdc_burn_parse_mac_ini_key_value(const char *setName, const char *keyName,
						    const char *usrKeyVal)
{
	int rc = 0;
	int grpN = 0;
	...
	rc = strncmp("Group", setName, 5) || (strlen(setName) != 6);
...
static int _update_mac_with_cfg_and_bootpart(KeyInf_mac_t *pKeyInf4Consist,
					     KeyInf_mac_t *pKeyInfInBootPart,
					     u8 *keyVal, unsigned int *keyValLen)
{
...
static int get_key_val_for_fmt_mac(const char *licPath, u8 *keyVal, unsigned int *keyValLen, const char *keyName)
{
...
static int update_lic_for_fmt_mac(const char *keyName, const char *licenseName)
{
	...
	rc = optimus_sdc_bootPart_lic_update_key_inf(keyName,
						     (u8 *)_KEYS_PRV_ITEM_INF_ADDR_4_CONSIST,
						     sizeof(KeyInf_mac_t));
	return rc;
}
#endif//MAC
...
int optimus_sdc_keysprovider_get_keyval(const void *pHdle, u8 *pBuf, unsigned int *keySz)
{
	...
	const char *keyName    = pMappedKey->keyName;
	const char  *licPath    = pMappedKey->licenseFile;
	int rc = 0;

	if (pMappedKey->magic != KEY_MAP_MAGIC) {
